...
from PyQt5.QtWidgets import (
	...
	QGridLayout, QLineEdit, QActionGroup)
from PyQt5.QtWidgets import (
	QStatusBar, QMenuBar, QToolBar
	#technically could've used the functions as they are inherited by QMainWindow
)
...
class appSettings(QSettings):
	def __init__(self):
		...
		# QSettings("D:\Projects\Python\SwitchBoard\MySwitchboard.cfg", QSettings.IniFormat)
		self.setPath(QSettings.IniFormat, QSettings.UserScope,"D:\Projects\Python\SwitchBoard\cfgClass_Test.cfg")
	def initSettings(self):
		# test=config.setValue("test", 3);    print("test=" + str(config.value("test")))
		# TODO: add config that stores all window geometry data
		# Style Configs
		cfgStyle = self.value("primaryStyle")
		if str(self.value("primaryStyle")).capitalize().replace(" ", "") not in validStyles:
			self.setValue("primaryStyle", "Windows Vista")
			print("Resetting style to hard default")

		# Main Toolbar Configs
		self.beginGroup("MainToolbar")
		cfgMainToolBarPos = self.value("mainToolBarPosition")
		if cfgMainToolBarPos == "\n" or cfgMainToolBarPos not in ["1", "2", "4", "8"]:
			self.setValue("mainToolBarPosition", Qt.LeftToolBarArea)  # Default toolbar position is on the left side
		print(type(self.value("mainToolBarPosition")))
		# Currently Non Functional
		cfgMainToolBarMoveable = self.value("isMainToolBarMovable")
		# if cfgMainToolBarMoveable == "\n" or type(cfgMainToolBarMoveable)!=bool:
		if cfgMainToolBarMoveable not in extendedBools:
			self.setValue("isMainToolBarMovable", True)  # Main toolbar is movable by default
		elif cfgMainToolBarMoveable in [1, "t", "T", "true", "True"]:
			self.setValue("isMainToolBarMovable", True)
		elif cfgMainToolBarMoveable in [0, "f", "F", "false", "False"]:
			self.setValue("isMainToolBarMovable", False)
		# Currently Non Functional
		cfgMainToolBarFloatable = self.value("isMainToolBarFloatable")
		if cfgMainToolBarFloatable not in extendedBools:
			self.setValue("isMainToolBarFloatable", True)  # Main toolbar is floatable by default
		elif cfgMainToolBarFloatable in [1, "t", "T", "true", "True"]:
			self.setValue("isMainToolBarFloatable", True)
		elif cfgMainToolBarFloatable in [0, "f", "F", "false", "False"]:
			self.setValue("isMainToolBarFloatable", False)
		self.endGroup()

		# Other Configs
		cfgTitle = self.value("cfgWindowTitle")
		if cfgTitle == "\n":     self.setValue("cfgWindowTitle", "I am a window")
...
class window(QMainWindow):
	def __init__ (self):
		...
		# self.restoreGeometry()TODO:THIS
		print("done init")
...
##This will deal with the settings page
	def appPreferences (self):
		...
		# add content to this
		#TODO: FIGURE OUT HOW TO FORCE THE SETTINGS PAGE TO REFRESH/RESET ITS CONTENTS ON REOPENING/DISCARDING CHANGES
	...
	# 	self.mainToolBar.defaultPos = Qt.LeftToolBarArea
	def initSettings(self):
		config= self.settings
		config.initSettings()
